...
const validateCommon = async (req, isUpdate = false) => {
  ...
  // company_id — required; a brand belongs to one company
  ...
  } else if (companyId && await brandRepository.fieldExists(companyId, 'code', body.code.trim().toUpperCase(), ignoreId)) {
    errors.push('This company already has a brand with this code.');
  }
  ...
  } else if (companyId && await brandRepository.fieldExists(companyId, 'name', body.name.trim(), ignoreId)) {
    errors.push('This company already has a brand with this name.');
  }
